...
# 페이지 설정
# ------------------------------------------------------------------
st.set_page_config(page_title="AD Lab AI Chat", layout="wide")
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("MCP 도구 상태")

    if st.button("🔄 도구 다시 불러오기"):
        st.session_state.tools_cache = None

    if st.session_state.tools_cache is None:
        try:
            st.session_state.tools_cache = list_tools()
        except Exception as exc:
            st.error(f"MCP 서버 연결 실패: {exc}")
            st.session_state.tools_cache = []

    for t in st.session_state.tools_cache or []:
        with st.expander(f"🔧 {t['name']}"):
            st.caption(t["description"][:300])
...
def run_agent_loop(user_text: str, model_id: str) -> str:
    tools = to_litellm_tools(st.session_state.tools_cache or [])
